...
export const useAuth = () => {
	...
	const signInWithGoogle = useCallback(async () => {
		try {
			...
			dispatch(
				setError((error as Error).message || 'Google sign in failed') as any
			);
			throw error;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const signInWithApple = useCallback(async () => {
		try {
			...
			dispatch(
				setError((error as Error).message || 'Apple sign in failed') as any
			);
			throw error;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const signInWithProvider = useCallback(
		async (provider: string) => {
			try {
				...
				dispatch(
					setError(
						(error as Error).message || `${provider} sign in failed`
					) as any
				);
				throw error;
			} finally {
	...
	const resetPassword = async (email: string) => {
		try {
			...
			dispatch(
				setError((err as Error).message || 'Password reset failed') as any
			);
			throw err;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const updateProfile = async (displayName: string, photoURL?: string) => {
		try {
			...
			dispatch(
				setError((err as Error).message || 'Profile update failed') as any
			);
			throw err;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const linkProvider = async (provider: string) => {
		try {
			...
			dispatch(
				setError((err as Error).message || 'Account linking failed') as any
			);
			throw err;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const unlinkProvider = async (providerId: string) => {
		try {
			...
			dispatch(
				setError((err as Error).message || 'Account unlinking failed') as any
			);
			throw err;
		} finally {
			dispatch(setLoading(false) as any);
	...
	const deleteAccount = useCallback(async () => {
		try {
			...
			dispatch(
				setError((error as Error).message || 'Account deletion failed') as any
			);
			throw error;
		} finally {
			dispatch(setLoading(false) as any);
