...
const PrivacyDashboard: React.FC = () => {
	...
	const [activeTab, setActiveTab] = useState<'consents' | 'data' | 'settings'>(
		'consents'
	);
	...
	const handleDataExport = async (format: 'json' | 'csv' | 'pdf') => {
		...
		try {
			...
			const requestId = await GDPRComplianceService.requestDataExport(
				user?.uid || '',
				format
			);
			...
			alert(
				`Data export requested. Request ID: ${requestId}. You will receive an email when it's ready.`
			);
		} catch (error) {
	...
	const handleDeletionRequest = async () => {
		...
		try {
			...
			const requestId = await GDPRComplianceService.requestDeletion(
				user?.uid || '',
				reason || undefined
			);

			alert(
				`Account deletion scheduled for ${format(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), 'PPP')}. You will receive an email with instructions to cancel if needed.`
			);
	...
	const handlePrivacySettingChange = async (
		...
		try {
			const updatedSettings = {
				...privacySettings,
				[category]: {
					...(privacySettings[category as keyof PrivacySettings] as object),
					[setting]: value,
				},
			};

			await GDPRComplianceService.updatePrivacySettings(
				user?.uid || '',
				updatedSettings
			);
				...
				<div className='space-y-4'>
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Control how we process your data. Some features may be limited
								if you opt out.
							</CardDescription>
						</CardHeader>
							...
							<div className='flex items-center justify-between py-3 border-b'>
								<div className='flex-1'>
									...
									<p className='text-sm text-gray-600 dark:text-gray-400'>
										Required for the app to function properly. Cannot be
										disabled.
									</p>
								</div>
				...
				<div className='space-y-4'>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Download a copy of your data (GDPR Article 20 - Data
								Portability)
							</CardDescription>
						</CardHeader>
						<CardContent>
									...
									<div className='flex'>
										...
										<div className='ml-3'>
											...
											<ul className='mt-1 text-sm text-red-700 dark:text-red-300 list-disc list-inside'>
												<li>
													All your 2FA accounts will be permanently deleted
												</li>
					...
					<Card>
						...
						<CardContent className='space-y-4'>
							...
							<div className='flex items-center justify-between py-3'>
								...
								<Switch
									checked={
										privacySettings.dataSharing.anonymizedDataForImprovement
									}
									onCheckedChange={(checked) =>
										handlePrivacySettingChange(
											'dataSharing',
											'anonymizedDataForImprovement',
											checked
										)
									}
								/>
