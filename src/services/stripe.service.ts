...
export class StripeService {
    ...
    try {
      ...
      if (result.subscription) {
        const firestoreResult = await FirestoreService.getCollection(
          'subscriptions',
          [{ field: 'providerSubscriptionId', operator: '==', value: params.subscriptionId }]
        );
   ...
   * Cancel subscription
   */
  static async cancelSubscription(subscriptionId: string, immediately: boolean = false): Promise<{
    success: boolean;
    subscription?: UserSubscription;
    error?: string;
  }> {
   ...
   */
  static async getInvoices(customerId: string, limit: number = 10): Promise<{
    success: boolean;
    invoices: Invoice[];
    error?: string;
  }> {
    try {
      const response = await fetch(`/api/stripe/invoices/${customerId}?limit=${limit}`);
      const result = await response.json();
   ...
   */
  static async createPortalSession(customerId: string, returnUrl: string): Promise<{
    success: boolean;
    url?: string;
    error?: string;
  }> {
  ...
  private static async handleSubscriptionUpdate(subscription: unknown): Promise<void> {
    try {
      const firestoreResult = await FirestoreService.getCollection(
        'subscriptions',
        [{ field: 'providerSubscriptionId', operator: '==', value: subscription.id }]
      );
  ...
  private static async handleInvoicePaymentFailed(invoice: unknown): Promise<void> {
    try {
      ...
      const firestoreResult = await FirestoreService.getCollection(
        'subscriptions',
        [{ field: 'providerSubscriptionId', operator: '==', value: invoice.subscription }]
      );
  ...
  private static async handlePaymentMethodAttached(paymentMethod: unknown): Promise<void> {
    try {
      ...
      const existing = await FirestoreService.getCollection(
        'payment_methods',
        [{ field: 'providerPaymentMethodId', operator: '==', value: paymentMethod.id }]
      );

      if (!existing.success || existing.data.length === 0) {
        await FirestoreService.addDocument('payment_methods', {
