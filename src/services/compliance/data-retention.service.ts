...
export class DataRetentionService {
	...
	private static readonly DEFAULT_POLICIES: Omit<
		RetentionPolicy,
	...
	static async upsertPolicy(
		...
		try {
			...
			if (policyId) {
				await updateDoc(
					doc(db, this.POLICIES_COLLECTION, policyId),
					policyData
				);
			} else {
		...
		try {
			...
			switch (policy.dataType) {
				...
				case DataType.DELETED_USERS:
					await this.processDeletedUsers(
						retentionDate,
						policy.action,
						(processed, succeeded, failed, error) => {
							itemsProcessed += processed;
			...
			await updateDoc(doc(db, this.POLICIES_COLLECTION, policyId), {
				...
				nextRun: Timestamp.fromDate(
					this.calculateNextRun(policy.retentionPeriod)
				),
			});
	...
	private static calculateNextRun(period: RetentionPeriod): Date {
		...
		if (period.unit === 'days' && period.value <= 30) {
			...
			period.unit === 'days' ||
			(period.unit === 'months' && period.value <= 6)
		) {
