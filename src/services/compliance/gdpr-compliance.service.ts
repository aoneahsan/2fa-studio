...
export class GDPRComplianceService {
			...
			await AuditHelper.logComplianceAction(
				...
				user.email!,
				{
					...
					dataTypes: Object.keys(exportRequest.includeData).filter(
						(key) =>
							exportRequest.includeData[
								key as keyof typeof exportRequest.includeData
							]
					),
				}
			);
	...
	static async requestDeletion(
		...
		try {
			...
			scheduledDate.setDate(
				scheduledDate.getDate() + this.DELETION_GRACE_PERIOD_DAYS
			);
	...
	static async cancelDeletion(
		...
		try {
			const requestDoc = await getDoc(
				doc(db, this.DELETION_REQUESTS, requestId)
			);
	 ...
	 */
	private static async executeDeleteion(
		...
		try {
			...
			if ((request as any).dataToDelete.backups) {
				...
				await Promise.all(
					(backupsList.items || []).map((item: any) => deleteObject(item))
				);
			}
			...
			await AuditHelper.logComplianceAction(
				...
				request.userEmail,
				{
					...
					dataDeleted: Object.keys(request.dataToDelete).filter(
						(key) =>
							request.dataToDelete[key as keyof typeof request.dataToDelete]
					),
				}
			);
		} catch (error) {
