...
export class DeviceService {
	...
	static async registerDevice(
		...
		const deviceName =
			name ||
			`${info.manufacturer || ''} ${info.model || 'Unknown Device'}`.trim();
		...
		return snapshot.docs.map(
			(doc: any) =>
				({
					...
					lastActive:
						(doc.data().lastActive as Timestamp)?.toDate() || new Date(),
					createdAt:
		...
		return snapshot.docs.map(
			(doc: any) =>
				({
					...
					...doc.data(),
					startedAt:
						(doc.data().startedAt as Timestamp)?.toDate() || new Date(),
					lastActivity:
						(doc.data().lastActivity as Timestamp)?.toDate() || new Date(),
					expiresAt:
						(doc.data().expiresAt as Timestamp)?.toDate() || new Date(),
				}) as DeviceSession
		);
	}
	...
	static async cleanupExpiredSessions(userId: string): Promise<void> {
		...
		const q = query(sessionsRef, where('expiresAt', '<', Timestamp.now()));
		const snapshot = await getDocs(q);
