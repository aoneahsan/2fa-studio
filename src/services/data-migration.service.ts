...
export class DataMigrationService {
		...
		for (const account of accounts.data) {
			try {
				...
				if (fromVersion === '1.0.0') {
					...
					decryptedSecret = atob(account.encryptedSecret as string);
				} else {
				...
				const newEncrypted =
					await MobileEncryptionService.encryptData(decryptedSecret);
				...
				console.error(
					`Failed to migrate encryption for account ${account.id}:`,
					error
				);
				throw error;
			}
	...
	private static async updateAccountSchema(
		...
		for (const account of accounts.data) {
			...
			if (fromVersion <= '1.1.0') {
				...
				updates.biometricTimeout = undefined;
				updates.lastBiometricAuth = undefined;
			}
	...
	private static async convertData(
		...
		if (stepId === 'migrate_folders') {
			...
			for (const account of accounts.data) {
				const category = account.category || 'default';
				const folderId = categoryMap.get(category as string) || null;
	...
	private static async attemptRollback(
		...
		console.log(
			`Attempting rollback for user ${userId}, completed steps: ${completedSteps}`
		);
	 ...
	 */
	private static validateAccountData(
		account: unknown,
		version: string
	): string[] {
		const errors: string[] = [];
