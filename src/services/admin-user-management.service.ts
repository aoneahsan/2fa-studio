...
export class AdminUserManagementService {
		...
		try {
			...
			const enhancedUsers = await Promise.all(
				(result.data || []).map((user: any) =>
					this.enhanceUserWithAdminData(user as User)
				)
			);
			...
			if (filter?.tier) {
				filteredUsers = filteredUsers.filter(
					(user: any) => user.subscription?.tier === filter.tier
				);
			}
	...
	static async unsuspendUser(
		...
		try {
			...
			// Send notification
			await this.sendUserNotification(userId, 'account_reactivated', {
				reason,
			});
		} catch (error) {
	...
	static async exportUserData(userId: string): Promise<{
		...
		try {
			...
			// Get user accounts
			const accountsResult = await FirestoreService.getCollection(
				`users/${userId}/accounts`
			);
			...
			const subscriptions = subscriptionsResult.success
				? (subscriptionsResult.data as UserSubscription[])
				: [];
	...
	private static async enhanceUserWithAdminData(
		user: User
	): Promise<AdminUser> {
		try {
			...
			const subscription =
				subscriptionsResult.success && subscriptionsResult.data.length > 0
			...
			const violations = violationsResult.success
				? violationsResult.data.length
				: 0;
	...
	private static async logUserAction(
		...
		try {
			...
			await FirestoreService.createDocument('user_actions', userAction);
		} catch (error) {
