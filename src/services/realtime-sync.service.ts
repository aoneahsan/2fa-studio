...
export class RealtimeSyncService {
	...
	private static pendingOperations: Map<string, Record<string, unknown>> =
		new Map();
	 ...
	 */
	private static async handleAccountsSnapshot(
	...
	private static async handleAccountChange(
		...
		if (changeType === 'removed') {
			...
			this.emitEvent({
				type: 'data_changed',
				data: { type: 'account_removed', account },
			});
			return;
		}
	 ...
	 */
	private static async handleFoldersSnapshot(
		snapshot: QuerySnapshot
	): Promise<void> {
		const changes = snapshot.docChanges();
	...
	private static async handleFolderChange(
		...
		if (changeType === 'removed') {
			await this.removeLocalFolder((folder as any).id);
		} else {
	 ...
	 */
	private static async handleTagsSnapshot(
		snapshot: QuerySnapshot
	): Promise<void> {
		const changes = snapshot.docChanges();
	...
	private static async handleTagChange(
		...
		if (changeType === 'removed') {
			await this.removeLocalTag((tag as any).id);
		} else {
	 ...
	 */
	private static async handleUserSnapshot(
	...
	static async resolveConflict(
		...
		try {
			...
			if (conflict.type === 'account') {
				await this.saveRemoteAccount(resolvedData as any);
				await this.saveLocalAccount(resolvedData as any);
			}
	 ...
	 */
	private static async getLocalAccount(
		accountId: string
