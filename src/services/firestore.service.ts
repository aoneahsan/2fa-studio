...
export class FirestoreService {
	...
	static async getCollection<T = any>(
		...
		try {
			...
			return { success: false, data: [], hasMore: false, fromCache: false };
		}
	}
		...
		try {
			...
			if (conflictResolution?.strategy === 'merge') {
				...
				if (currentDoc.exists()) {
					...
					const mergedData = this.mergeDocuments(
						currentData,
						data,
	...
	static async getUserAccounts(
		...
		return this.getCollection<OTPAccount>(`users/${userId}/accounts`, [], {
	...
	static async getUserDevices(userId: string): Promise<SyncResult<Device>> {
		return this.getCollection<Device>(`users/${userId}/devices`, [], {
	...
	static async getUserBackups(userId: string): Promise<SyncResult<any>> {
		return this.getCollection(`users/${userId}/backups`, [], {
			orderByField: 'createdAt',
			orderDirection: 'desc',
			limit: 50,
		});
