	...
	static async createSubscriptionFromPurchase(
		...
		try {
			...
			const subscription: UserSubscription = {
				...
				createdAt: new Date(),
				updatedAt: new Date(),
				trialStart: purchase.isTrialPeriod
					? new Date(purchase.transactionDate)
					: undefined,
				trialEnd: purchase.isTrialPeriod
					? new Date(purchase.transactionDate + 7 * 24 * 60 * 60 * 1000)
					: undefined,
				metadata: {
	...
	static async handleServerNotification(
		...
		try {
			...
			switch (notificationType) {
				...
				default:
					console.log(
						`Unhandled App Store notification type: ${notificationType}`
					);
			}
		} catch (error) {
