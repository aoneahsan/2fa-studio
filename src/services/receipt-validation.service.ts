...
export class ReceiptValidationService {
	 ...
	 */
	private static async validateStripeReceipt(
		request: ValidationRequest
	): Promise<ValidationResult> {
		try {
			...
			return {
				...
				fraudRisk: 'high',
				error:
					error instanceof Error ? error.message : 'Stripe validation error',
			};
		}
	}
	...
	private static async validateGooglePlayReceipt(
		...
		try {
			...
			const subscription: UserSubscription = {
				...
				status: this.mapGooglePlayStatus(
					validation.subscription?.purchaseState || 0
				),
				provider: 'google_play',
				providerSubscriptionId: request.receiptData,
				currentPeriodStart: new Date(
					validation.subscription?.startTimeMillis || Date.now()
				),
				currentPeriodEnd: new Date(
					validation.subscription?.expiryTimeMillis || Date.now()
				),
	 ...
	 */
	private static async validateAppStoreReceipt(
		request: ValidationRequest
	): Promise<ValidationResult> {
		try {
			...
			const subscription: UserSubscription = {
				...
				tier: this.getTierFromProductId(
					(validation as any).purchase.productIdentifier
				),
				status: this.mapAppStoreStatus(validation.purchase),
				provider: 'app_store',
				providerSubscriptionId: (validation as any).purchase
					.transactionIdentifier,
				currentPeriodStart: new Date(
					(validation as any).purchase.transactionDate
				),
				currentPeriodEnd: new Date(
					validation.purchase.subscriptionExpirationDate || Date.now()
				),
				cancelAtPeriodEnd: !(validation as any).purchase
					.subscriptionAutoRenewStatus,
				createdAt: new Date(),
				updatedAt: new Date(),
				trialStart: validation.purchase.isTrialPeriod
					? new Date((validation as any).purchase.transactionDate)
					: undefined,
				trialEnd: validation.purchase.isTrialPeriod
					? new Date(
							validation.purchase.transactionDate + 7 * 24 * 60 * 60 * 1000
						)
					: undefined,
				metadata: {
					transactionIdentifier: (validation as any).purchase
						.transactionIdentifier,
					originalTransactionIdentifier: (validation as any).purchase
			...
			return {
				...
				fraudRisk: 'high',
				error:
					error instanceof Error ? error.message : 'App Store validation error',
			};
		}
	}
		...
		try {
			...
			if (userSubscriptions.success) {
				const providers = new Set(
					(userSubscriptions.data || []).map((sub: any) => sub.provider)
				);
				if (providers.size > 2) {
					score += 20;
	 ...
	 */
	private static async getRecentValidations(
		userId: string,
		timeWindowSeconds: number
	): Promise<any[]> {
		const cutoff = new Date(Date.now() - timeWindowSeconds * 1000);

		const result = await FirestoreService.getCollection('validation_records', [
	...
	private static async storeValidationRecord(
		...
		try {
			await FirestoreService.createDocument('validation_records', {
				userId: request.userId,
	 ...
	 */
	private static mapGooglePlayStatus(
