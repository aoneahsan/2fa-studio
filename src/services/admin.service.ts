...
export class AdminService {
  ...
  static async searchUsers(params: UserSearchParams): Promise<{
    ...
    try {
      ...
      if (params.subscriptionTier) {
        constraints.push(where('subscription.tier', '==', params.subscriptionTier));
      }
      if (params.subscriptionStatus) {
        constraints.push(where('subscription.status', '==', params.subscriptionStatus));
      }
   ...
   */
  private static getFeaturesByTier(tier: SubscriptionTier): Record<string, boolean> {
    const baseFeatures = {
      cloudBackup: false,
  ...
  static async sendUserNotification(params: {
    ...
    try {
      const sendNotification = httpsCallable(this.functions, 'sendUserNotification');
  ...
  static async exportUserData(userId: string): Promise<string> {
    try {
      const exportData = httpsCallable<{ userId: string }, { downloadUrl: string }>(
        this.functions, 
        'exportUserData'
      );
