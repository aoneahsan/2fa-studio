...
export class ImportExportService {
	 ...
	 */
	private static async exportTo2FAS(
		accounts: OTPAccount[],
		password?: string
	): Promise<string> {
	 ...
	 */
	private static async exportToAegis(
		accounts: OTPAccount[],
		password?: string
	): Promise<string> {
		const aegisData = {
		...
		if (password) {
			...
			console.warn(
				'Password encryption for Aegis format is not fully implemented'
			);
		}
	 ...
	 */
	private static async exportToAndOTP(
		accounts: OTPAccount[],
		password?: string
	): Promise<string> {
		...
		if (password) {
			...
			console.warn(
				'Password encryption for andOTP format is not fully implemented'
			);
		}
	 ...
	 */
	private static async importFrom2FAS(
		data: string,
		password?: string
		...
		try {
			...
			for (const accountData of (parsedData as any).accounts) {
				try {
					...
					result.failed++;
					result.errors.push(
						`Failed to import ${accountData.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
					);
				}
			}
			...
			result.errors.push(
				`Failed to parse 2FAS data: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	 ...
	 */
	private static async importFromAegis(
		data: string,
		password?: string
	): Promise<ImportResult> {
		const result: ImportResult = {
		...
		try {
			...
			// Validate format
			if (
				!parsedData.db?.entries ||
				!Array.isArray((parsedData as any).db.entries)
			) {
			...
			for (const entry of (parsedData as any).db.entries) {
				try {
					...
					result.failed++;
					result.errors.push(
						`Failed to import ${entry.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
					);
				}
			}
	 ...
	 */
	private static async importFromAndOTP(
		data: string,
		password?: string
	): Promise<ImportResult> {
		const result: ImportResult = {
		...
		try {
			...
			result.errors.push(
				`Failed to parse andOTP data: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	...
	private static importFromGoogleAuth(data: string): ImportResult {
		...
		for (const line of lines) {
			try {
				...
				result.failed++;
				result.errors.push(
					`Failed to import URI: ${error instanceof Error ? error.message : 'Unknown error'}`
				);
			}
		}
			...
			try {
				...
				for (const line of lines) {
					...
					switch (key.toLowerCase().trim()) {
						...
						case 'algorithm':
						case 'algo':
							account.algorithm = value.toUpperCase() as
								| 'SHA1'
								| 'SHA256'
								| 'SHA512';
							break;
				...
				result.failed++;
				result.errors.push(
					`Failed to import entry: ${error instanceof Error ? error.message : 'Unknown error'}`
				);
			}
		}
