...
export class AdminAnalyticsService {
	...
	static async getDashboardMetrics(
		...
		try {
			const [userMetrics, revenueMetrics, usageMetrics] = await Promise.all([
			...
			const metrics: DashboardMetrics = {
				...
				activeSubscriptions: Object.values(userMetrics.usersByTier).reduce(
					(a, b) => a + b,
					0
				),
	...
	static async getUserMetrics(filter?: AnalyticsFilter): Promise<UserMetrics> {
		try {
			...
			const activeUsersResult = await FirestoreService.getCollection('users', [
				{ field: 'lastLoginAt', operator: '>', value: monthAgo },
			]);
			const activeUsers = activeUsersResult.success
				? activeUsersResult.data.length
				: 0;
		...
		try {
			...
			const monthlyRevenue =
				invoices
					.filter(
						(invoice: any) =>
							invoice.status === 'paid' && invoice.paidAt >= startOfMonth
					)
			...
			const yearlyRevenue =
				invoices
					.filter(
						(invoice: any) =>
							invoice.status === 'paid' && invoice.paidAt >= startOfYear
					)
					.reduce((sum, invoice) => sum + invoice.amount, 0) / 100;
			...
			const activeSubscriptions = activeSubscriptionsResult.success
				? activeSubscriptionsResult.data.length
				: 1;
	...
	static async getUsageMetrics(
		...
		try {
			...
			const averageAccountsPerUser =
				uniqueUsers > 0 ? totalAccounts / uniqueUsers : 0;
			const averageBackupsPerUser =
				uniqueUsers > 0 ? totalBackups / uniqueUsers : 0;
	...
	static async getTimeSeriesData(
		...
		try {
			...
			const startDate =
				filter?.startDate ||
				new Date(endDate.getTime() - this.getPeriodDuration(period));
	...
	private static async getUsersByTier(): Promise<
	...
	private static async getTopCountries(): Promise<
	...
	private static calculateConversionRate(userMetrics: UserMetrics): number {
		...
		return userMetrics.totalUsers > 0
			? (paidUsers / userMetrics.totalUsers) * 100
			: 0;
	}

	private static async getRevenueByTier(
		invoices: unknown[]
	...
	private static async calculateRevenueGrowth(): Promise<{
		monthly: number;
		yearly: number;
	}> {
	...
	private static async calculateLifetimeValue(): Promise<number> {
		...
		return 89.5; // Average LTV in dollars
	}
