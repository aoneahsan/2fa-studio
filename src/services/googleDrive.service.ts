...
export class GoogleDriveService {
	...
	static async authenticate(): Promise<boolean> {
		...
		return new Promise((resolve) => {
			(this.tokenClient as any).callback = async (response: any) => {
	...
	static async listBackups(): Promise<GoogleDriveFile[]> {
		...
		try {
			...
			const response = await gapi.client.drive.files.list({
				...
				fields:
					'files(id, name, mimeType, createdTime, modifiedTime, size, properties)',
				pageSize: 100,
	...
	static async syncBackup(
		...
		try {
			...
			if (backups.length > 0) {
				...
				const latestBackup = await this.getBackup(
					backups[0].id,
					encryptionPassword
				);
