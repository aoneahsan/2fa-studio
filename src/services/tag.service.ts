...
export class TagService {
	...
	static async createTag(
		...
		if (
			existingTags.some((t) => t.name.toLowerCase() === tag.name.toLowerCase())
		) {
	...
	static async updateTag(
		userId: string,
		tagId: string,
		updates: Partial<Omit<Tag, 'id' | 'userId' | 'createdAt'>>
	): Promise<void> {
	 ...
	 */
	static async removeTagsFromAccount(
		userId: string,
		accountId: string,
		tagIds: string[]
	): Promise<void> {
	 ...
	 */
	static async getAccountsByTags(
		userId: string,
		tagIds: string[],
		mode: 'AND' | 'OR' = 'OR'
	): Promise<Account[]> {
		...
		if (mode === 'OR') {
			...
			return accounts.filter((account: any) =>
				account.tags?.some((tag: string) => tagIds.includes(tag))
			);
		} else {
	 ...
	 * Update tag counts
	 */
	private static async updateTagCounts(
		userId: string,
		tagIds: string[],
		delta: number
	): Promise<void> {
	...
	static getTagSuggestions(issuerName: string): string[] {
		...
		if (
			lowerIssuer.includes('bank') ||
			lowerIssuer.includes('paypal') ||
			lowerIssuer.includes('stripe')
		) {
		...
		if (
			...
			lowerIssuer.includes('twitter') ||
			lowerIssuer.includes('instagram')
		) {
			suggestions.push('Social');
		}

		if (
			lowerIssuer.includes('amazon') ||
			lowerIssuer.includes('ebay') ||
			lowerIssuer.includes('shopify')
		) {
			suggestions.push('Shopping');
		}

		if (
			lowerIssuer.includes('github') ||
			lowerIssuer.includes('gitlab') ||
			lowerIssuer.includes('bitbucket')
		) {
