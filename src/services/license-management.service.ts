...
export class LicenseManagementService {
	...
	private static usageCache: Map<string, { usage: Usage; timestamp: number }> =
		new Map();
	...
	static async enforceFeatureAccess(
		...
		const check = await this.checkFeatureAccess(
			userId,
			feature,
			usageIncrement
		);
	...
	static async generateUsageReport(
		...
		try {
			...
			const usage: Usage =
				usageResult.success && usageResult.data.length > 0
			...
			const overage = {
				accounts: Math.max(
					0,
					usage.accounts -
						(license.limits.accounts === -1
							? 0
							: (license as any).limits.accounts)
				),
				backups: Math.max(
					0,
					usage.backups -
						(license.limits.backups === -1
							? 0
							: (license as any).limits.backups)
				),
			...
			const cost =
				overage.accounts * 0.1 + // $0.10 per extra account
	...
	private static async checkViolations(
		...
		if (limits.accounts !== -1 && usage.accounts > limits.accounts) {
			violations.push(
				`Account limit exceeded: ${usage.accounts}/${limits.accounts}`
			);
		}

		// Check backups limit
		if (limits && typeof limits === 'object' && 'backups' in limits) {
			if (usage.backups > (limits as any).backups) {
				violations.push(
					`Backup limit exceeded: ${usage.backups}/${(limits as any).backups}`
				);
			}
		}
	...
	private static async logViolation(
		...
		try {
			...
			if (violation.severity === 'critical') {
				...
				console.warn(
					`Critical license violation for user ${userId}:`,
					violation
				);
			}
		} catch (error) {
	 ...
	 */
	private static async getCurrentSubscription(
		userId: string
				...
				[
					{
						field: 'status',
						operator: 'in',
						value: ['active', 'trialing', 'past_due'],
					},
				]
			);
	 ...
	 */
	private static getMinimumTierForFeature(
		feature: keyof SubscriptionFeatures
	): SubscriptionTier {
