...
export class ProvisioningAPIService {
  ...
  static async initializeProvisioning(
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, this.CONFIG_COLLECTION), provisioningConfig);
  ...
  static async updateProvisioningConfig(
    teamId: string,
    updates: Partial<Omit<ProvisioningConfig, 'id' | 'teamId' | 'createdAt' | 'apiKeys'>>,
    updatedBy: string
  ): Promise<void> {
    try {
      // Check permissions
  ...
  static async addApiKey(
    ...
    try {
      ...
      const apiKey = await this.generateApiKey(name, createdBy, permissions, options?.expiresInDays);
      ...
      await this.logProvisioningActivity(teamId, 'api_key_created', 'user', undefined, {
        keyId: apiKey.id,
        keyName: name
      }, 'success');
  ...
  static async revokeApiKey(
    ...
    try {
      ...
      await this.logProvisioningActivity(teamId, 'api_key_revoked', 'user', undefined, {
        keyId,
  ...
  static async createSCIMUser(
    ...
    try {
      ...
      const extension = user['urn:2fastudio:schemas:extension:enterprise:2.0:User'];
      if (extension?.roleId) {
      ...
      if (extension?.vaultIds) {
        for (const vaultId of extension.vaultIds) {
          await TeamVaultService.addMemberToVault(vaultId, authUser.uid, 'system');
        }
      }
      ...
      await this.logProvisioningActivity(teamId, 'user_created', 'user', authUser.uid, {
        email: user.emails[0].value,
  ...
  static async updateSCIMUser(
    ...
    try {
      ...
      const extension = updates['urn:2fastudio:schemas:extension:enterprise:2.0:User'];
      if (extension?.roleId) {
        ...
        await RBACService.assignRole(userId, extension.roleId, 'system', teamId);
      }
    ...
    try {
      ...
      if (group.members) {
        for (const member of group.members) {
          if (member.type === 'User') {
            await TeamVaultService.addMemberToVault(vaultId, member.value, 'system');
          }
        }
      }
      ...
      await this.logProvisioningActivity(teamId, 'group_created', 'group', vaultId, {
   ...
   */
  static async getTeamProvisioningConfig(teamId: string): Promise<ProvisioningConfig | null> {
    try {
      const q = query(
