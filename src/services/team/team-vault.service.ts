...
export class TeamVaultService {
  ...
  static async createVault(
    vault: Omit<TeamVault, 'id' | 'createdAt' | 'updatedAt' | 'memberIds' | 'accountIds'>,
    creatorId: string
  ): Promise<string> {
    try {
      // Check permissions
      ...
      const docRef = await addDoc(collection(db, this.VAULTS_COLLECTION), vaultData);

      await this.logVaultAccess(docRef.id, undefined, creatorId, VaultAction.VAULT_CREATED, {
      ...
      console.error('Failed to create vault:', _error);
      throw error;
    }
  }
  ...
  static async updateVault(
    vaultId: string,
    updates: Partial<Omit<TeamVault, 'id' | 'createdAt' | 'createdBy' | 'teamId'>>,
    userId: string
  ): Promise<void> {
    try {
      ...
      if (vault.settings.requireApproval && !vault.settings.approvers?.includes(userId)) {
      ...
      await this.logVaultAccess(vaultId, undefined, userId, VaultAction.VAULT_UPDATED, {
      ...
      console.error('Failed to update vault:', _error);
      throw error;
    }
  }
  ...
  static async addAccountToVault(
    ...
    try {
      ...
      await addDoc(collection(db, this.VAULT_ACCOUNTS_COLLECTION), vaultAccount);
      ...
      await this.logVaultAccess(vaultId, accountId, userId, VaultAction.ACCOUNT_ADDED, {
      ...
      console.error('Failed to add account to vault:', _error);
      throw error;
    }
  }
  ...
  static async removeAccountFromVault(
    ...
    try {
      ...
      if (!hasPermission.allowed) {
        throw new Error('Insufficient permissions to remove account from vault');
      }
      ...
      await this.logVaultAccess(vaultId, accountId, userId, VaultAction.ACCOUNT_REMOVED, {});
      ...
      console.error('Failed to remove account from vault:', _error);
      throw error;
    }
  }
  ...
  static async addMemberToVault(
    ...
    try {
      ...
      console.error('Failed to add member to vault:', _error);
      throw error;
    }
  }
  ...
  static async removeMemberFromVault(
    ...
    try {
      ...
      console.error('Failed to remove member from vault:', _error);
      throw error;
    }
  }
  ...
  static async getVault(vaultId: string): Promise<TeamVault | null> {
    try {
      ...
      console.error('Failed to get vault:', _error);
      throw error;
    }
  }
   ...
   * Get team vaults
   */
  static async getTeamVaults(teamId: string, userId: string): Promise<TeamVault[]> {
    try {
      // Check permissions
      const hasPermission = await RBACService.checkPermission(
        userId,
      ...
      console.error('Failed to get team vaults:', _error);
      throw error;
    }
  }
  ...
  static async getVaultAccounts(vaultId: string): Promise<VaultAccount[]> {
    try {
      ...
      console.error('Failed to get vault accounts:', _error);
      throw error;
    }
  }
  ...
  static async accessVaultAccount(
    ...
    try {
      ...
      console.error('Failed to access vault account:', _error);
      throw error;
    }
  }
  ...
  static async requestApproval(
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, this.VAULT_APPROVALS), approval);

      await this.logVaultAccess(vaultId, targetId, requestedBy, VaultAction.APPROVAL_REQUESTED, {
        action,
      ...
      console.error('Failed to request approval:', _error);
      throw error;
    }
  }
  ...
  static async processApproval(
    ...
    try {
      const approvalDoc = await getDoc(doc(db, this.VAULT_APPROVALS, approvalId));
      ...
      console.error('Failed to process approval:', _error);
      throw error;
    }
  }
  ...
  static async getVaultAccessLogs(
    ...
    try {
      ...
      console.error('Failed to get vault access logs:', _error);
      throw error;
    }
  }
  ...
  private static async logVaultAccess(
    ...
    try {
      ...
      console.error('Failed to log vault access:', _error);
    }
  }

  private static async executeApprovedAction(approval: VaultApproval): Promise<void> {
