...
export class RBACService {
  ...
  private static readonly SYSTEM_ROLES: Omit<Role, 'id' | 'createdAt' | 'updatedAt'>[] = [
    {
      name: 'super_admin',
      ...
      permissions: [
        {
          resource: Resource.ACCOUNTS,
          actions: [Action.READ],
          conditions: [{ type: 'custom', field: 'shared', operator: 'equals', value: true }]
        },
        {
          resource: Resource.VAULTS,
          actions: [Action.READ],
  ...
  static async initializeRoles(): Promise<void> {
    try {
      const rolesSnapshot = await getDocs(collection(db, this.ROLES_COLLECTION));
  ...
  static async assignRole(
    ...
    try {
      ...
      const canGrant = await this.checkPermission(grantedBy, Resource.TEAM_ROLES, Action.UPDATE, { teamId });
      if (!canGrant.allowed) {
      ...
      const docRef = await addDoc(collection(db, this.USER_ROLES_COLLECTION), userRole);
  ...
  static async revokeRole(
    ...
    try {
      ...
      const canRevoke = await this.checkPermission(revokedBy, Resource.TEAM_ROLES, Action.UPDATE, { teamId });
      if (!canRevoke.allowed) {
        throw new Error('Insufficient permissions to revoke roles');
      }

      const assignment = await this.getUserRoleAssignment(userId, roleId, teamId);
      if (!assignment) {
  ...
  static async createRole(
    ...
    try {
      ...
      const canCreate = await this.checkPermission(createdBy, Resource.TEAM_ROLES, Action.CREATE);
      ...
      const docRef = await addDoc(collection(db, this.ROLES_COLLECTION), roleData);
  ...
  static async updateRole(
    ...
    try {
      ...
      const canUpdate = await this.checkPermission(updatedBy, Resource.TEAM_ROLES, Action.UPDATE);
  ...
  static async deleteRole(roleId: string, deletedBy: string): Promise<void> {
    try {
      ...
      const canDelete = await this.checkPermission(deletedBy, Resource.TEAM_ROLES, Action.DELETE);
  ...
  private static async getUserRoles(userId: string, teamId?: string): Promise<UserRole[]> {
    let q = query(
  ...
  private static matchesResource(permissionResource: Resource, requestedResource: Resource): boolean {
    // Exact match
    ...
    for (const condition of conditions) {
      switch (condition.type) {
        ...
        case 'custom':
          ...
          const value = context.customConditions[condition.field];
