...
export async function aggregateDailyStats() {
  try {
    ...
    await db.collection("analytics").doc(today.toISOString().split("T")[0]).set(stats);
...
function calculateSummary(docs: admin.firestore.QueryDocumentSnapshot[]) {
  ...
  summary.accounts.netGrowth = summary.accounts.totalCreated - summary.accounts.totalDeleted;
  
  summary.backups.total = totalBackups;
  summary.backups.automaticPercentage = totalBackups > 0 ? (automaticBackups / totalBackups) * 100 : 0;
